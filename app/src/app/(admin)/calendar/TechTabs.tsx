...
export function TechTabs({
  ...
  current: string; // "all" or tech id
  techs: TechOption[];
}) {
  const entries: { key: string; label: string; icon?: React.ReactNode }[] = [
    { key: "all", label: "全部", icon: <Users className="h-4 w-4" /> },
    ...techs.map((t) => ({ key: t.id, label: t.name })),
  ];
  const currentIdx = Math.max(
    0,
    entries.findIndex((e) => e.key === current),
  );
  const prev = entries[(currentIdx - 1 + entries.length) % entries.length];
  const next = entries[(currentIdx + 1) % entries.length];

  return (
    <div className="flex items-center gap-2">
      <Link
        href={`/calendar?tech=${prev.key}`}
